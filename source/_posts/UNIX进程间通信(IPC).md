...
#### 进程间通信 
由于虚拟内存的关系，对于所有进程来说，它们只能看到操作系统想让他们看到的虚拟地址空间，这块空间是每个进程的私有财产，这部分私 直接导致程序崩溃，因此，无论出于什么目的，想要访问一块不属于进程自身的地址空间时，就需要使用到进程间通信了。
...
cmdstring为需要执行的shell命令，而type是“r”，则返回的文件指针是可读的，而“w”返回的文件指针是可写的。

含有一个函数`pclose`可以用于关闭标准I/O流，等待命令的终止，返回shell的终止状态。它的函数原型是：
